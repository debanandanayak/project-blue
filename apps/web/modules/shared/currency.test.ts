...
describe('currency', () => {
	describe('formatAmountInCents', () => {
		...
		test('the currency can be changed', () => {
			expect(
				formatAmountInCents({ amountInCents: 1000, currency: 'EUR' }),
			).toBe('€10.00')
			expect(
				formatAmountInCents({ amountInCents: 1000, currency: 'JPY' }),
			).toBe('¥10.00')
			expect(
				formatAmountInCents({ amountInCents: 1000, currency: 'GBP' }),
			).toBe('£10.00')
		})
	})
})
