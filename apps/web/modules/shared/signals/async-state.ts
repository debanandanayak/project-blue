...
function useAsyncState<Args, Return, ReturnFormatted = Return>(
	getter: (args: Args) => Promise<Return>,
	{
		...
		initialData?: ReturnFormatted
		formatValue?: (value: Return) => ReturnFormatted
		immediate?: boolean
	} = {},
) {
	const [getIsLoading, setIsLoading] = createSignal(false)
	const [getError, setError] = createSignal<Error | undefined>(undefined)
	const [getData, setData] = createSignal<ReturnFormatted | undefined>(
		initialData,
	)
	...
	const finishHook = createHook<{
		data: ReturnFormatted | undefined
