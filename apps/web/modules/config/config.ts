...
export const buildTimeConfig = {
  ...
  vitrineBaseUrl: asString(process.env.VITE_VITRINE_BASE_URL, 'http://localhost:3000/'),
  isDemoMode: asBoolean(process.env.VITE_IS_DEMO_MODE, false),
  auth: {
    isRegistrationEnabled: asBoolean(process.env.VITE_AUTH_IS_REGISTRATION_ENABLED, true),
    isPasswordResetEnabled: asBoolean(process.env.VITE_AUTH_IS_PASSWORD_RESET_ENABLED, true),
    isEmailVerificationRequired: asBoolean(process.env.VITE_AUTH_IS_EMAIL_VERIFICATION_REQUIRED, true),
    showLegalLinksOnAuthPage: asBoolean(process.env.VITE_AUTH_SHOW_LEGAL_LINKS_ON_AUTH_PAGE, false),
    providers: {
      email: { isEnabled: asBoolean(process.env.VITE_AUTH_PROVIDERS_EMAIL_IS_ENABLED, true) },
      github: { isEnabled: asBoolean(process.env.VITE_AUTH_PROVIDERS_GITHUB_IS_ENABLED, false) },
      google: { isEnabled: asBoolean(process.env.VITE_AUTH_PROVIDERS_GOOGLE_IS_ENABLED, false) },
      customs: [] as {
        providerId: string;
        providerName: string;
  ...
  documents: {
    deletedDocumentsRetentionDays: asNumber(process.env.VITE_DOCUMENTS_DELETED_DOCUMENTS_RETENTION_DAYS, 30),
  },
  posthog: {
  ...
  isSubscriptionsEnabled: asBoolean(process.env.VITE_IS_SUBSCRIPTIONS_ENABLED, false),
} as const;
